...
class UsersFirestore {
  ...
  Future<bool> existeCorreoEnContactos(String? userId, String email) async {
    try {
      ...
      CollectionReference contactosCollection =
          usersCollection.doc(userId).collection('contacts');
  ...
  Future<Map<String, String>> obtenerUsuarioActual(String userId) async {
    DocumentSnapshot usuario = await usersCollection.doc(userId).get();
  ...
  Future<List<Map<String, String>>> obtenerContactosDisponibles(
      String userId) async {
    try {
      // Obtener referencia
      ...
      CollectionReference contactosCollection =
          usersCollection.doc(userIdEncontrado).collection('contacts');
